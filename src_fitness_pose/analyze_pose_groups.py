...
def write_markdown(performance: pd.DataFrame, flow: pd.DataFrame, ranking: pd.DataFrame, output_dir: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "## 논문용 해석 문장",
            "",
            "> 자세 그룹별 분석 결과, 모델 성능은 운동군에 따라 차이를 보였다. 런지 계열, 누운 자세 코어 운동, 상지 지지/엎드린 자세 운동은 세부 라벨 간 관절 움직임이 유사하여 상대적으로 오분류가 많이 발생하였다. 반면 버피 테스트나 고관절 힌지 운동처럼 다른 운동군과 자세 구조가 비교적 뚜렷한 경우에는 높은 분류 성능을 보였다. 이는 관절 좌표 기반 특징이 큰 자세군의 차이는 효과적으로 구분하지만, 같은 자세군 내부의 세밀한 운동 차이를 구분하는 데에는 한계가 있음을 보여준다.",
        ]
    )
